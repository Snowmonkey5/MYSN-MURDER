...
export class GameRoom {
  ...
  handleMessage(ws, raw) {
    ...
    const { type, data } = msg;
    switch (type) {
      case 'join': this.join(ws, data); break;
      case 'settings': this.updateSettings(ws, data); break;
      case 'start': this.startGame(ws); break;
      case 'move': this.move(ws, data); break;
      case 'kill': this.kill(ws, data); break;
      case 'report': this.report(ws); break;
      case 'emergency': this.emergency(ws); break;
      case 'task': this.completeTask(ws, data); break;
      case 'vote': this.vote(ws, data); break;
  ...
  join(ws, { name, code, hostOnly }) {
    ...
    if (room.players.size >= room.settings.maxPlayers) return this.send(ws, { type: 'error', data: 'Room is full' });

    const id = crypto.randomUUID();
    const used = [...room.players.values()].map(p => p.color);
    const color = hostOnly ? '#1C9455' : (COLORS.find(c => !used.includes(c)) || COLORS[Math.floor(Math.random() * COLORS.length)]);

    const player = {
      id,
      name: (name || (hostOnly ? 'Host' : 'Player')).slice(0, 12),
      color,
      x: 400,
      y: 300,
      role: null,
  ...
  updateSettings(ws, settings) {
    ...
    if (!session || !this.room || this.room.hostId !== session.id || this.room.state !== 'lobby') return;
  ...
  startGame(ws) {
    ...
    // Only real players (not hostOnly) get roles
    ...
    // Host only players stay without role
  ...
  doKill(killer, victim) {
    ...
    this.room.bodies.push({
      id: victim.id, name: victim.name, color: victim.color, x: victim.x, y: victim.y
    });
  ...
  vote(ws, { targetId }) {
    ...
    this.broadcast({
      ...
      data: {
        ...
        total: [...this.room.players.values()].filter(p => p.alive && !p.hostOnly).length
      }
    });
  }
  ...
  checkTaskWin() {
    ...
    if (crew.every(c => c.completedTasks >= (c.tasks.length || 1))) this.endGame('crew');
  }
